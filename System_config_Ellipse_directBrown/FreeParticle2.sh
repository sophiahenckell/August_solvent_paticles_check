...
DATA="/data/scc/sophia/FreeParticleSystemsRadius1/FreeParticle/${RUN_ID}-n${PARTICLE_NUMBER}-t${SIMULATION_TIME}-phi${PACKING_FRACTION}-k${ASPECT_RATIO}-tb${BROWNIAN_TIMESTEP}-ts${SAVING_TIMESTEP}-r${RADIUS}"
mkdir -p $DATA/logs
...
cat > $TMPFILE <<- EOF
	...
	echo "Free particle Ellipse radius 1 $JOB_ID.\$SGE_TASK_ID $DATA" >> /data/scc/sophia/globallogs/clustersubmit.log
