...
NAME="FREE1Particle${RUN_ID}T${SIMULATION_TIME}N${PARTICLE_NUMBER}k${ASPECT_RATIO//.}Phi${PACKING_FRACTION//.}"
DATA="/data/scc/sophia/FreeParticleSystemsRadius05/FreeParticle/${RUN_ID}-n${PARTICLE_NUMBER}-t${SIMULATION_TIME}-phi${PACKING_FRACTION}-k${ASPECT_RATIO}-tb${BROWNIAN_TIMESTEP}-ts${SAVING_TIMESTEP}-r${RADIUS}"
mkdir -p $DATA/logs
...
cat > $TMPFILE <<- EOF
	...
	echo "Free particle Ellipse radius 05 $JOB_ID.\$SGE_TASK_ID $DATA" >> /data/scc/sophia/globallogs/clustersubmit.log
